...
# Page 4: Primary Treatment
    ...
    # ✅ CHANGE 1: Replace OPENROUTER_API_KEY with NVIDIA_API_KEY
    NVIDIA_API_KEY = st.secrets.get("NVIDIA_API_KEY", None)
    if not NVIDIA_API_KEY:
        st.error("API key not found. Please set NVIDIA_API_KEY in Streamlit secrets.")
        st.stop()
    ...
    # (All CSS stays exactly the same — no changes needed)
    st.markdown(
        """
        <style>
        .user-message {
