...
def Prediction():
# Load models
   #bayes, logistic, tree,random_tree, svm_linear, svm_rbf, svm_sigmoid = load_models()

    # User input
    # user_input = st.text_input('Enter your Name:', 'Your input here')
    bayes = pickle.load(open('Save models/bayes.pkl', 'rb'))
    logistic = pickle.load(open('Save models/logistic.pkl', 'rb'))
    random_tree = pickle.load(open('Save models/random_tree.pkl', 'rb'))
    svm_linear = pickle.load(open('Save models/svm_linear.pkl', 'rb'))
    svm_rbf = pickle.load(open('Save models/svm_rbf.pkl', 'rb'))
    svm_sigmoid = pickle.load(open('Save models/svm_sigmoid.pkl', 'rb'))
    tree = pickle.load(open('Save models/tree.pkl', 'rb'))
...
if submit_button:
    # st.write(f"Predicted Results, {Prediction()}!")
    #st.write(f" First Variable,{combined_df}!")
    st.write("Combined DataFrame:")
    st.write(combined_df)
    st.write(Prediction())
    #st.write(f" First Variable,{combined_df.iloc[:,[6]]}!")
    #st.write(f" First Variable,{new_data.iloc[:,[2]]}!")
    #st.write(f" First Variable,{combined_df.iloc[:,[6]]}!")
    #st.write(f" First Variable,{new_data.iloc[:,[4]]}!")
